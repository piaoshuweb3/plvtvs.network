...
//
// Strategy: Render a HIDDEN ConnectButton (to initialize RainbowKit's
// modal context), then use useConnectModal() hook to open the modal.
// This is the most reliable approach — the hook is always available
// once the hidden ConnectButton is mounted.
// ============================================================
...
export default function WalletButton({ compact = false }: WalletButtonProps) {
  ...
  const handleConnectClick = () => {
    ...
    if (openConnectModal) {
      ...
      setError('Wallet not ready. Please retry.');
    }
  };
  ...
  return (
    <>
      ...
      {isConnecting || (authenticating && !error) ? (
        <button
          disabled
        ...
        <div className="relative">
          ...
          >
            <span
              ...
              style={{
                background: user.role === 'SUPER_ADMIN' ? '#FFCC00' : user.role === 'OPERATOR' ? '#0066FF' : '#00FFCC',
                boxShadow: `0 0 6px ${user.role === 'SUPER_ADMIN' ? '#FFCC00' : user.role === 'OPERATOR' ? '#0066FF' : '#00FFCC'}`,
              }}
            />
            <span className="flex-1 text-left min-w-0">
              <div className="text-white truncate">{`${address.slice(0, 6)}...${address.slice(-4)}`}</div>
              {!compact && (
                <div className="text-[7px] sm:text-[8px] text-[#666] tracking-wider">
                  {user.role === 'SUPER_ADMIN' ? 'SUPER_ADMIN' : user.role === 'OPERATOR' ? 'OPERATOR' : user.subscriptionTier > 0 ? 'SUBSCRIBER' : 'GHOST'}
                </div>
              )}
            </span>
            ...
            <>
              ...
              <div className="absolute right-0 top-full mt-1 z-50 w-52 sm:w-64 bg-black border border-[#1a1a1a]">
                <div className="p-3 border-b border-[#1a1a1a]">
                  <div className="cyber-mono text-[9px] text-[#444] tracking-wider mb-1">AUTHENTICATED GHOST</div>
                  <div className="cyber-mono text-xs text-[#00FFCC] break-all">{`${address.slice(0, 6)}...${address.slice(-4)}`}</div>
                </div>
                <div className="py-1">
                  {pathname !== '/' && (
                    <button onClick={() => goTo('/')} className="w-full text-left px-3 py-2 cyber-mono text-[11px] text-[#bbb] hover:text-[#00FFCC] hover:bg-[#00FFCC11] transition-colors flex items-center gap-2">
                      <span>▣</span> HOME
                    </button>
                  )}
                  {pathname !== '/dashboard' && (
                    <button onClick={() => goTo('/dashboard')} className="w-full text-left px-3 py-2 cyber-mono text-[11px] text-[#bbb] hover:text-[#00FFCC] hover:bg-[#00FFCC11] transition-colors flex items-center gap-2">
                      <span>◉</span> DASHBOARD
                    </button>
                  )}
                  {(isOperator() || isSuperAdmin()) && pathname !== '/admin' && (
                    <button onClick={() => goTo('/admin')} className="w-full text-left px-3 py-2 cyber-mono text-[11px] text-[#FFCC00] hover:bg-[#FFCC0011] transition-colors flex items-center gap-2">
                      <span>⊞</span> ADMIN
                    </button>
                  )}
                  <div className="border-t border-[#1a1a1a] my-1" />
                  <button onClick={() => { setDropdownOpen(false); openAccountModal?.(); }} className="w-full text-left px-3 py-2 cyber-mono text-[11px] text-[#888] hover:text-[#00FFCC] hover:bg-[#00FFCC11] transition-colors flex items-center gap-2">
                    <span>⟳</span> SETTINGS
                  </button>
                  <button onClick={handleDisconnect} className="w-full text-left px-3 py-2 cyber-mono text-[11px] text-[#ff4444] hover:bg-[#ff444411] transition-colors flex items-center gap-2">
