...
// ============================================================
// WalletButton — uses ConnectButton.Custom render prop
// The ConnectButton is ALWAYS mounted so the modal context
// is always available via the render prop's openConnectModal.
// ============================================================
...
export default function WalletButton({ compact = false }: WalletButtonProps) {
  ...
  useEffect(() => {
    ...
    if (lastAuthWalletRef.current !== walletLower && !authenticating && !isReconnecting) {
  ...
  return (
    <ConnectButton.Custom>
      {({ account, chain, openConnectModal, openAccountModal, openChainModal, mounted: rkMounted }) => {
        ...
        if (!rkMounted) {
          return (
            <button disabled className="cyber-btn cyber-btn-blue !py-2 !px-2 sm:!px-4 !text-[9px] sm:!text-[10px] opacity-50">
        ...
        if (isConnecting || (authenticating && !error)) {
          return (
            <button disabled className="cyber-btn cyber-btn-blue !py-2 !px-2 sm:!px-4 !text-[9px] sm:!text-[10px] cursor-wait">
        ...
        // Disconnected — clicking calls openConnectModal
        if (!account || !isConnected) {
          return (
            <div className="relative">
              ...
              {error && (
                <div className="absolute right-0 top-full mt-1 cyber-mono text-[9px] text-[#ff4444] z-50 max-w-[260px]">
                  {error}
                </div>
              )}
            </div>
          ...
          return (
            <div className="relative">
              <button
                ...
                className="cyber-btn !py-2 !px-2 sm:!px-4 !text-[9px] sm:!text-[10px]"
        ...
        const roleColor = role === 'SUPER_ADMIN' ? '#FFCC00' : role === 'OPERATOR' ? '#0066FF' : '#00FFCC';
        const roleLabel = role === 'SUPER_ADMIN' ? 'SUPER_ADMIN' : role === 'OPERATOR' ? 'OPERATOR' : user.subscriptionTier > 0 ? 'SUBSCRIBER' : 'GHOST';
        ...
        return (
          <div className="relative">
            ...
            >
              <span className="w-1.5 h-1.5 rounded-full cyber-pulse flex-shrink-0" style={{ background: roleColor, boxShadow: `0 0 6px ${roleColor}` }} />
              <span className="flex-1 text-left min-w-0">
                <div className="text-white truncate">{short}</div>
                {!compact && <div className="text-[7px] sm:text-[8px] text-[#666]">{roleLabel}</div>}
              </span>
