  ...
  try {
    ...
    if (isUsingMemDb || !db) {
      ...
      if (!user) {
        ...
        mdb.logs.push({ id: mdb.genId(), userId: user.id, sector: 'SYSTEM', level: 'SUCCESS', message: `New ghost: ${walletAddress.slice(0, 10)}...`, metadata: null, createdAt: new Date() });
      } else {
    ...
    if (!user) {
      ...
      await db.activityLog.create({ data: { userId: user.id, sector: 'SYSTEM', level: 'SUCCESS', message: `New ghost: ${walletAddress.slice(0, 10)}...` } });
    } else {
      if (user.status === 'BANNED') return NextResponse.json({ error: 'Banned' }, { status: 403 });
      if (user.status === 'SUSPENDED') return NextResponse.json({ error: 'Suspended' }, { status: 403 });
      const role = deriveRoleFromWallet(walletAddress);
